...
import ljdp.minechem.common.items.ItemPeriodicTable;
...
public class MinechemItems {
	...
	public static ItemPeriodicTable periodicTable;
	...
	private static int periodicTableID;
	...
	public static void loadConfig(Configuration config) {
		elementID   = config.getItem(config.CATEGORY_ITEM, "Element", 4736).getInt(4736);
	 	...
	 	periodicTableID = config.getItem(config.CATEGORY_ITEM, "PeriodicTable", 4743).getInt(4743);
	}
	...
	public static void registerItems() {
		...
		periodicTable = new ItemPeriodicTable(periodicTableID);
		...
		LanguageRegistry.addName(periodicTable, MinechemHelper.getLocalString("item.name.periodicTable"));
